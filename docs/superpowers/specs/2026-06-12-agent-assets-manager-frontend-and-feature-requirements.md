...
MVP 可以先用列表或紧凑卡片实现，但必须保留未来做安装矩阵的空间。
...
- PATH 中的 `codex`、`claude`、`opencode`、`hermes`、`openclaw`
- `/opt/homebrew/bin`
- `/usr/local/bin`
- `~/.local/bin`
- `~/.codex`
...
第一版实现建议先打通 `CodexAdapter` 的读路径，再扩展其他适配器。
...
- 左侧导航可切换 7 个页面。
- 资产页筛选可工作。
- 平台页可展示至少 Codex、Claude Code、OpenCode、Hermes、OpenClaw。
- 模型页不显示任何完整 API Key。
...
1. 搭建 Tauri 2 + React + TypeScript + Vite 工程骨架。
2. 建立中文 UI design tokens 和基础组件。
3. 实现静态中文版概览页，严格贴近原型图。
4. 抽出 mock 数据类型，和未来 SQLite/adapter 返回结构对齐。
5. 实现左侧导航和 7 个页面空壳。
6. 实现资产筛选、平台详情面板、操作预览弹窗。
7. 接入 SQLite schema。
8. 实现 `CodexAdapter` 读路径。
9. 将概览页从 mock 数据切到真实扫描数据。
10. 扩展 Claude Code、OpenCode、Hermes、OpenClaw 读路径。

实现 Agent 不应跳过第 3 步。先把中文原型界面做对，再接真实扫描能力。
